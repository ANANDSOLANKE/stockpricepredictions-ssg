...
function clearActive(parent) {
  [...parent.querySelectorAll("button")].forEach(b => b.classList.remove(activeStyle));
}
...
async function loadSite() {
  const res = await fetch(`${window.BASE_URL}/data/index.json`, {cache: "no-store"});
...
/* Light geolocation: try ipapi.co; if matches a country slug, preselect it */
  ...
  try{
    ...
    for (const [rslug, rdata] of Object.entries(SITE)) {
      if (rdata.countries[cslug]) {
        ...
        if (exchs.length) {
          // Click through programmatically
          selectRegionSlug(rslug);
          selectCountrySlug(rslug, cslug);
